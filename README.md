...
| 任意 1D | NN 拟合 | 透射/反射 | 梯度有限差分校验 |

## 功能边界与已知限制

- **从头算 PES 未实现**: `AbInitioData` 仅为数据容器, 无 ASE/PySCF 后端。
- **NN 拟合为基础版**: 一维输入; 无归一化持久化、无力训练; 早停已恢复
  最优权重。0.3.1 修复的线性输出层已包含在本版 (旧模型预测会改变, 需重训)。
